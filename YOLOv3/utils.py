...
def parse_data_config(dataset_name, path):
    ...
    for line in lines:
        ...
        if line.startswith('['):
            ...
            # print(line)
            key, value = line.split('=')
            configs[tmp_name][key.strip()] = value.strip()
    return configs[dataset_name] if dataset_name in configs else print(f'No {dataset_name} config')
    ...
    for sample_i in range(len(outputs)):
        ...
        if len(annotations):
            ...
            for pred_i, (pred_box, pred_label) in enumerate(zip(pred_boxes, pred_labels)):
                ...
                box_iou, box_index = bbox_iou(pred_box.unsqueeze(0), target_boxes).max(0)
                iou_boxes.append(box_iou)
                # print(f'iou: {iou}')
                if box_iou >= iou_threshold and box_index not in detected_boxes:
                    true_positives[pred_i] = 1
                    detected_boxes += [box_index]
        
        batch_metrics.append([true_positives, pred_scores, pred_labels, iou_boxes])
