...
def plot_img_and_bbox(img_path, detections, detect_size, class_names, output_dir, save_plot=True):
    ...
    if detections is None:
        ...
        detections = rescale_boxes(detections, detect_size, img.shape[:2])
...
def detect_img(img_path, model, device, detect_size, class_names, conf_thres, nms_thres, output_dir, save_plot=True):
    ...
    # print(f'inference_time: {datetime.timedelta(seconds = inference_time)}')

    if detections:
        # it is a list due to the implementation of non_max_suppression that deal with batch samples
        plot_img_and_bbox(img_path, detections[0], detect_size, class_names, output_dir, save_plot)
    else:
        print('No veins detected on this image!')
...
if __name__ == "__main__":
    ...
    count = 0
    for i, fn in enumerate(os.listdir(opt.image_folder)):
        if not fn.endswith('.jpg'):
            continue
        count += 1
        
        img_path = os.path.join(opt.image_folder, fn)
        detections, inference_time = detect_img(img_path, model, device, opt.img_size, classes, opt.conf_thres, opt.nms_thres, output_dir, save_plot=True)
